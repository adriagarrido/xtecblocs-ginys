...
class XTEC_My_Favorites extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( count( $blogs ) > 0 ) {
			...
			foreach ( $blogs as $blog ) {
				...
				$titol_blog = trim( get_blog_option( $blog_id, 'blogname' ) );
				$url_blog   = trim( $url_blog, '/' );

				if ( empty( $titol_blog ) ) {
					$url_blog   = get_blog_option( $blog_id, 'siteurl' );
					$titol_blog = 'Bloc ' . substr( $url_blog, strrpos( $url_blog, '/' ) + 1 );
				} else {
					$titol_blog = stripslashes( get_blog_option( $blog_id, 'blogname' ) );
					?>
					<li>
						<a href="<?php echo esc_url( get_blogaddress_by_id( $blog_id ) ); ?>" target="_blank" title="Entra al bloc">
							<?php echo esc_html( $titol_blog ); ?>
						</a>
						&nbsp;
						<a href="<?php echo esc_url( 'index.php?action=delete_favorite&blog_id=' . $blog_id . '&xg_key=' . wp_create_nonce( 'xg_key' ) ); ?>" title="Esborra">
	 ...
	 * Undocumented function
	 *
	 * @param [type] $blog_id No comment yet.
	 * @return void
	 */
	public function xg_add_favorite( $blog_id ) {
	 ...
	 * @return array
	 */
	public function xg_get_favorites() {
		...
		$blogs = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT blogId FROM {$wpdb->base_prefix}user_blogs WHERE userId = %d",
				$current_user->ID
			),
			ARRAY_A
		); // db call ok. no-cache ok.
