...
function getWeekBlog() {
	...
	$blog = $wpdb->get_col(
		"SELECT blogName " .
		"FROM wp_weekblog " .
		"WHERE endPublish >= CURRENT_DATE() " .
		"AND initPublish <= CURRENT_DATE() " .
		"AND active=1 " .
		"ORDER BY endPublish limit 0,1"
	);
	$blogd = $wpdb->get_col(
		"SELECT description " .
		"FROM wp_weekblog " .
	...
	$blogId = $wpdb->get_results(
		"SELECT blog_id, domain, path " .
		"from $wpdb->blogs " .
		"WHERE path like '". PATH_CURRENT_SITE ."$blog[0]/'"
	);
	...
	$thispost = $wpdb->get_results(
		"SELECT post_title, guid, post_content, post_date, post_author " .
		"FROM $blogPostsTable " .
